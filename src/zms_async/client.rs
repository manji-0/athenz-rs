    ...
    fn apply_auth(&self, req: RequestBuilder) -> Result<RequestBuilder, Error> {
        common::apply_auth(req, &self.auth, |req, header, value, ctx| {
            let header_name = HeaderName::from_bytes(header.as_bytes()).map_err(|e| {
                let msg = match ctx {
                    common::AuthContext::Config => {
                        format!("invalid header name (config): {}", e)
                    }
                    common::AuthContext::NToken => format!("invalid header name: {}", e),
                };
                Error::Crypto(msg)
            })?;
            let header_value = HeaderValue::from_str(value).map_err(|e| {
                let msg = match ctx {
                    common::AuthContext::Config => {
                        format!("invalid header value (config): {}", e)
                    }
                    common::AuthContext::NToken => format!("invalid header value: {}", e),
                };
                Error::Crypto(msg)
            })?;
